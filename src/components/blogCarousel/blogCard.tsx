...
const BlogCard = ({ type, date, title, content, imageUrl, id }) => {
      ...
      <Link href={`${currentUrl}blog/${id}`}>
        ...
        <Card className="blog-card max-w-md w-full p-4 h-full border-none rounded-2xl transition-colors duration-300">
          ...
          <CardContent className="p-4 space-y-2">
            <div className="blog-card-date text-sm">{date}</div>
            <h2 className="blog-card-title text-lg font-semibold">{title}</h2>
