...
meta:
  ...
  - property: "og:image"
    content: "./img/blogs/ba_data_cover_image.png"
  - name: "twitter:card"
...
![Makes the abstract definition concrete. The reader just learned that MBAs analyze from the outside and data engineers work from inside. This image shows that structural difference visually before the argument begins.](./img/same-spike-two-reactions.png)
...
Every metric in a business - revenue, churn, CAC, LTV, retention, is produced by a pipeline. That pipeline has JOIN conditions, aggregation logic, filter predicates, and data source assumptions baked into it. When the pipeline has a bug, or when upstream data quality degrades, or when the definition of a metric silently changes because someone modified a dbt model, the number in the dashboard changes too.
...
![Image shwoing split of data vs business](./img/data-engineer-time-split.png)
...
![Purpose: The table above lists scenarios side by side. An image here reinforces the core cognitive difference visually — the same chart, two completely different mental models firing when they see it. Placed right after the table, it makes the table's point stick.](./img/same-spike-two-reactions.png)
...
![This is the structural pivot of the post — the moment the argument stops being one-sided and names what the best version looks like. A Venn diagram here does more work than three paragraphs. It's the image most likely to get shared on LinkedIn.](./img/ideal-analyst-venn.png)
...
![The section argues the role is changing fast. A timeline makes that trajectory visceral — it shows where the role started, where it is now, and where it's going. Hiring managers reading this section will feel this more than read it.](./img/ba-evolution-timeline.png)
...
- "Walk me through a time when you questioned a metric that the business was relying on. What did you find?"
- "How do you validate that a number in a dashboard is trustworthy before you use it in a recommendation?"
- "Describe a business question you were asked that turned out to be unanswerable with existing data. What did you do?"
- "How would you explain your most complex analysis to someone who has never seen the underlying data?"
