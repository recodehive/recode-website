  ...
  function removeMobileThemeToggle() {
    ...
    function hideThemeToggle() {
      ...
      if (sidebarBrand) {
        ...
        buttons.forEach((button, index) => {
          ...
          const hasThemeIcon = button.querySelector('svg') &&
          ...
          if (hasThemeIcon && !isCloseButton) {
            ...
            console.log('🤔 Potential theme toggle (removing to be safe):', button);
            button.remove();
            found = true;
          }
        });
      ...
      if (sidebar) {
        const themeButtons = sidebar.querySelectorAll('button[aria-label*="mode"], button[aria-label*="theme"], button[aria-label*="dark"], button[aria-label*="light"]');
        themeButtons.forEach(button => {
          if (!button.classList.contains('navbar-sidebar__close')) {
            console.log('🎯 Found theme button via fallback, removing:', button);
            button.remove();
            found = true;
          }
        });
    ...
    const interval = setInterval(() => {
      ...
      if (found || attempts >= maxAttempts) {
        ...
        if (found) {
          console.log('✅ Successfully removed theme toggle from mobile sidebar');
        }
      }
    }, 500);
        ...
        if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
          ...
          mutation.addedNodes.forEach(function(node) {
            if (node.nodeType === 1) { // Element node
              if (node.classList?.contains('navbar-sidebar') || 
